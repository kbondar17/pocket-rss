...
#    def __init__(self, page_count, current_page=1, data_pattern={}):
    ...
    def _to_button_array(self, keyboard_dict):
        ...
        if type(self.data_pattern) == str:
            ...
            for key in keys:
                self.data_pattern.update({'page': key})
                my_calldata = [str(e) for e in self.data_pattern.values()]
                my_calldata = ':'.join(my_calldata)

                keyboard.append(
                    InlineKeyboardButton(
                        text=str(keyboard_dict[key]),
                        callback_data=my_calldata
                    )
                )
            return _buttons_to_dict(keyboard)
